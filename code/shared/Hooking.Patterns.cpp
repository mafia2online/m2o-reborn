...
{
    auto file = Utility::MakeAbsolutePathW(L"data\\pt_map.dat");

    FILE *hints = nullptr;
    _wfopen_s(&hints, file.c_str(), L"ab");
...
#if PATTERNS_USE_HINTS
static nomad::base_function initFunction([] ()
{
    auto file = Utility::MakeAbsolutePathW(L"data\\pt_map.dat");
	FILE* hints = _wfopen(file.c_str(), L"rb");
