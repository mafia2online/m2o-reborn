...
namespace M2
{
    ...
    namespace Wrappers
    {
        ...
        public:
            ...
            {
                if (!m_pModelManager) {
                    m2sdk_log("[GameModelManager]: Null pointer\n");
                    return false;
                }
        ...
        public:
            ...
            GameModelManager    *Load(const char *directory, const char *model)
            {
                ...
                if (mgr != nullptr) {
                    m2sdk_log("[ModelManager]: Using preallocated model (size %d)\n", m_pModelManagers.size());
                    return mgr;
                }

                m2sdk_log("[ModelManager]: Using a new model (Size %d)\n", m_pModelManagers.size());

                if (m_pModelManagers.size() >= MODELMGR_MAX) {
                    m2sdk_log("[ModelManager]: Max models error\n");
                    return nullptr;
                }

                GameModelManager *pModelManager = new GameModelManager();
                if (!pModelManager) {
                    m2sdk_log("[ModelManager]: GameModelManager null pointer\n");
                    return nullptr;
                }

                if (!pModelManager->Load(directory, model)) {
                    delete(pModelManager);
                    m2sdk_log("[ModelManager]: Can't load model\n");
                    return nullptr;
                }
