...
{
	CGraphicsManager& graphics = CCore::Instance().GetGraphics();

	graphics.OnDeviceLost( m_pD3DDevice );

	HRESULT hResult = m_pD3DDevice->Reset( pPresentationParameters );

	if( SUCCEEDED( hResult ) )
	{
		graphics.OnDeviceReset( m_pD3DDevice, pPresentationParameters);
	}
...
HRESULT STDMETHODCALLTYPE CDirect3DDevice9Proxy::BeginScene( )
{
	...
	CCore::Instance().GetGraphics().OnDevicePreRender();
...
{
	CCore::Instance().GetGraphics().OnDeviceRender();
